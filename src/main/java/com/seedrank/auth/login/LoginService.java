...
class LoginService {
    ...
    private final PointRewardService pointRewards;
    private final String dummyPasswordHash;

    LoginService(UserRepository users, AuthSessionRepository sessions, PasswordEncoder passwords, TokenIssuer tokens,
            Clock clock, PointRewardService pointRewards) {
        this.users = users;
    ...
    LoginResult login(LoginRequest request) {
        ...
        boolean passwordMatches = passwords.matches(
                request.password(), candidate.map(User::getPasswordHash).orElse(dummyPasswordHash));
        User candidateUser = candidate.orElseThrow(InvalidCredentialsException::new);
        if (candidateUser.getStatus() != User.Status.ACTIVE || !passwordMatches) {
        ...
        pointRewards.grantDailyFirstAccess(user.getId());
        return new LoginResult(
                new LoginResponse(tokens.access(user, session.id(), now), "Bearer", 900, user.getId(), user.getRole()),
                refresh);
    }
