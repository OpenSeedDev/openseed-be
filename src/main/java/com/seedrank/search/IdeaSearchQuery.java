...
@Repository
class IdeaSearchQuery {
    private final JdbcTemplate jdbc;

    IdeaSearchQuery(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    List<RankingCardResponse> findCurrentRanking(String searchTerm) {
        ...
        return jdbc.query("""
                ...
                WHERE idea.status = 'PUBLISHED'
                  AND (
                      idea.title ILIKE ? ESCAPE '\\'
                      OR EXISTS (
                          SELECT 1
                          FROM idea_keywords keyword
                          WHERE keyword.idea_id = idea.id
                            AND keyword.keyword ILIKE ? ESCAPE '\\'
                      )
                  )
