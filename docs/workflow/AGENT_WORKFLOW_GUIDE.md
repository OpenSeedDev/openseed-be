...
- Fast Build에서는 집중 테스트를 통과한 선행 PR branch에서 후속 Stacked PR을 시작한다.
...
5. 백로그에서 실행 가능한 Delivery Lane과 Stacked parent를 계산한다.
6. 선택한 작업 하나를 독립 worktree에서 직접 구현한다.
7. PR 생성 후 tick을 종료하며 두 번째 작업은 다음 수동 tick까지 시작하지 않는다.
8. 테스트, 문서, 상태와 PR 증거를 검증한다.
9. 새 PR, 리뷰 반영, Merge, `BLOCKED`, 사용자 결정 필요가 있을 때만 보고한다.
...
변경에 관련 위험 테스트 묶음을 실행하고, stack depth 4 checkpoint와 리뷰·병합 직전
