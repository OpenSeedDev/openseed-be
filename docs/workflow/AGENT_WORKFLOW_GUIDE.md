...
현재 임시 Fast Build 모드에서는 에이전트가 모든 VS 구현 PR을 먼저 준비한다. 사용자는
Build 단계 종료 후 Stacked PR을 선행 순서로 리뷰한다. 모든 VS에 구현 PR이 생기면
동일한 수동 tick이 기존 리뷰·복구·병합 워크플로우로 자동 복귀한다.
...
- Fast Build에서는 기존 Stack leaf를 통합한 전역 기준 브랜치와 직전 전역 PR에서 후속
  Stacked PR을 시작한다.
...
에이전트 정의와 중첩 위임 설정이 적용되어 있다. 현재 예약 Automation은 중지되어 있으며,
사용자의 `다음 tick 실행`이 Coordinator에게 한 번의 tick을 위임한다.

Fast Build Build 단계에서 `다음 tick 실행`은 신규 VS 구현을 우선한다. `리뷰 tick 실행`은
원할 때만 열린 PR의 리뷰 수정을 처리한다. 모든 VS 작업에 Merge 또는 열린 구현 PR이
존재하면 `다음 tick 실행`은 기존 리뷰·복구·순차 병합 흐름으로 자동 전환된다.
...
2. 일반 모드에서는 리뷰 수정과 실패 복구를 우선한다. Fast Build Build 단계에서는 신규
   VS 구현을 우선하고 리뷰는 명시적인 리뷰 tick에 처리한다.
3. `/merge-approved`의 작성자, 댓글 시각과 승인 대상 코드 SHA를 검증한다.
4. 실제 Merge를 확인하고 `main`, 선행 조건과 resource lock을 동기화한다.
5. 전역 기준 브랜치가 없으면 기존 VS Stack leaf들을 통합하고 전체 테스트 후 Push한다.
6. 기준 브랜치가 있으면 실행 가능한 VS 하나와 전역 Stacked parent를 계산한다.
7. 선택한 작업 하나를 독립 worktree에서 직접 구현한다.
8. PR 생성 후 tick을 종료하며 두 번째 작업은 다음 수동 tick까지 시작하지 않는다.
9. 테스트, 문서, 상태와 PR 증거를 검증한다.
10. 새 PR, 리뷰 반영, Merge, `BLOCKED`, 사용자 결정 필요가 있을 때만 보고한다.
...
- 일반 작업은 최신 `origin/main`, Stacked 작업은 지정된 parent PR branch에서 독립
  worktree와 브랜치를 사용한다.
...
- Fast Build는 집중·위험 테스트를 로컬에서 실행하고 최종 Head CI로 전체 회귀를 검증한다.
  로컬 전체 테스트는 stack checkpoint와 최종 리뷰 안정화에서 실행한다.
...
flowchart TD
  ...
  B --> C{"모든 VS에 구현 PR이 있는가?"}
  C -- "아니요" --> T{"전역 통합 브랜치가 있는가?"}
  T -- "아니요" --> U["기존 Stack leaf 통합·전체 테스트·Push·종료"]
  T -- "예" --> J["Ready 작업 1개 선택"]
  J --> K{"선택 가능한 작업이 있는가?"}
  K -- "예" --> L["Coordinator가 단일 worktree에서 구현"]
  L --> M["TDD부터 PR까지 완료"]
  M --> N["tick 종료"]
  K -- "아니요" --> Z["변경 없이 tick 종료"]
  C -- "예" --> R["기존 수동 리뷰·복구·병합 모드"]
```

Fast Build의 Build 단계 처리 우선순위는 다음과 같다.
...
Build 종료 후에는 `리뷰 수정·실패 복구 → 병합 Gate → 실제 Merge → child base 갱신`
순서의 기존 수동 워크플로우로 돌아간다.
...
flowchart LR
  ...
  G --> RF["리팩터링"]
  RF --> FT["집중 테스트"]
  FT --> RISK{"Fast Build checkpoint인가?"}
  RISK -- "예" --> AT["로컬 전체 테스트"]
  RISK -- "아니요" --> RT["관련 위험 테스트"]
  AT --> DOC["OpenAPI·ERD·Migration"]
  RT --> DOC
...
Red·Green·리팩터링 중에는 관련 테스트만 실행한다. Fast Build에서는 다음 고위험
변경에 관련 위험 테스트 묶음을 실행하고, 전역 stack depth 4 간격 checkpoint와 리뷰·병합 직전
안정화에서 로컬 전체 검증을 실행한다.
...
## 단일 작업 실행과 resource lock

한 tick에는 다음 제한이 적용된다.

1. `backlog.yml`의 `max_parallel_workers = 1`
   - 한 tick에서 실제 구현 작업을 정확히 하나만 실행한다.
2. Coordinator가 해당 작업을 직접 처리한다.
   - 구현용 sub-agent를 병렬 생성하지 않는다.
3. PR 생성 후 tick을 종료한다.
   - 다음 작업은 다음 `다음 tick 실행`에서 다시 선택한다.

`AWAITING_PR_REVIEW` 상태의 PR은 실행 슬롯을 차지하지 않는다. Fast Build에서는 지정된
Stack 후속 작업만 같은 resource lock을 승계할 수 있다.
...
일반 모드에서는 VS-009가 Merge된 뒤 VS-010을 시작한다. Fast Build에서는 VS-009가
집중 테스트를 통과한 열린 PR이면 그 branch에서 VS-010을 Stacked PR로 만들 수 있다.

잠금이 겹치지 않는 Ready 작업이 여러 개여도 한 tick에서는 정렬상 첫 작업 하나만
처리한다. 나머지는 다음 수동 tick까지 대기한다.

## PR 리뷰와 비동기 수정

사용자는 여러 PR을 순서대로 리뷰할 수 있다. 리뷰 수정도 한 tick에 PR 하나씩 처리한다.
...
Fast Build 중 사용자 역할은 두 단계로 나뉜다.

1. Build 단계에서는 `다음 tick 실행`으로 구현 PR을 쌓고 `/merge-approved`는 남기지 않는다.
2. Build 종료 후 안내된 stack 선행 순서로 리뷰하고, 수정 댓글과 `/merge-approved`를 남긴다.
...
- 일반 모드에서 선행 PR이 Merge되기 전에 후속 기능을 시작하지 않는다.
- Fast Build에서도 집중 테스트를 통과한 지정 parent PR이 아닌 임의 branch에서 후속 기능을 시작하지 않는다.
...
- 선행 작업 Merge 증거 또는 Stacked parent PR·branch·Head SHA
- 알려진 제한
- 자동 리뷰 반영 정책
...
└── vertical-slice-coordinator
    └── Ready 작업 1개 구현 → PR 생성 → tick 종료
```

예약 Automation은 중지되어 있다. 사용자의 수동 명령이 Coordinator를 깨우고, 구체적인
판단은 Agent → Skill → 백로그·상태·GitHub 순서로 수행한다. Hook은 현재 구성에 포함되지
않으며 에이전트 실행에 필수도 아니다.
...
- `.agents/skills/vertical-slice/references/fast-build-policy.md`
