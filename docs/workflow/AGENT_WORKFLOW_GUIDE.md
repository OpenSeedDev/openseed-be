...
- PR을 최신 main으로 갱신하고 현재 Head 검사가 성공한 뒤 사용자가 정확한 `/merge-approved` 댓글을 남겨야 Merge가 진행된다.

## 현재 적용 상태

2026-07-22 기준으로 다음 구성이 `main`에 적용되어 있다.
...
에이전트 정의와 중첩 위임 설정이 적용되어 있다. 예약 Automation은 현재 중지되어 있으며,
사용자가 `다음 tick 실행`을 요청하면 Coordinator가 한 번의 orchestrator tick에서 독립
Ready 작업을 최대 3개까지 Worker에 위임한다.
...
- 주기 실행: 필요하면 `openseed` Automation을 별도로 활성화
- 수동 실행: 사용자가 `다음 tick 실행`으로 Coordinator 실행을 요청
...
3. 리뷰 준비 PR을 최신 main으로 갱신하고 새 Head 필수 검사의 성공을 확인한다.
4. 검사 성공 이후 작성된 `/merge-approved`의 작성자, 댓글 시각과 승인 대상 코드 SHA를 검증한다.
5. 실제 Merge를 확인하고 `main`, 선행 조건과 resource lock을 동기화한다.
6. 백로그에서 실행 가능한 독립 작업을 계산한다.
7. 선택한 작업마다 Worker 하나와 독립 worktree를 배정한다.
8. Worker가 반환한 테스트, 문서, 상태와 PR 증거를 검증한다.
...
flowchart TD
  ...
  C -- "아니요" --> U{"PR이 최신 main인가?"}
  U -- "아니요" --> V["main 최신화·새 Head 검사"]
  V --> N
  U -- "예" --> F{"검사 성공 뒤 유효한 /merge-approved인가?"}
...
- PR base SHA가 최신 main SHA와 같다.
- 현재 SHA의 필수 CI가 성공한 이후에 댓글이 작성되었다.
- 미해결 review thread가 없다.
- 선행 작업이 실제 Merge되었다.
- PR에 충돌이 없다.
- 활성 실패가 없다.

승인 후 코드가 Push되거나 main이 더 최신으로 변경되면 기존 auto-merge가 자동 해제된다. 에이전트가 branch를 다시 최신화하고 검사가 성공하면 사용자는 변경된 코드를 확인한 뒤 새로운 `/merge-approved` 댓글을 남겨야 한다.
...
3. PR이 최신 main이고 필수 검사가 성공한 뒤 현재 코드로 병합해도 되면 일반 댓글에 정확히 `/merge-approved`를 남긴다.
...
예약 Automation을 다시 켜더라도 Coordinator를 깨우는 역할만 맡고, 구체적인 판단은
Agent → Skill → 백로그·상태·GitHub 순서로 수행한다. Hook은 현재 구성에 포함되지 않으며
에이전트 실행에 필수도 아니다. 반복적으로 발생하는 위험 행동이 확인될 때만 명령 전후의
추가 강제 장치로 검토한다.
