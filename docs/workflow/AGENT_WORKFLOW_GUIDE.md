...
- 테스트를 먼저 실패시키고 최소 구현, 리팩터링, 전체 테스트 순서로 진행한다.
...
- 전체 테스트와 상태 검증을 통과한 뒤 Commit, Push와 PR 생성을 수행한다.
...
승인 후 코드가 Push되면 기존 승인은 자동으로 낡은 것으로 처리한다. 사용자는 변경된 코드를 다시 확인하고 새로운 `/merge-approved` 댓글을 남겨야 한다.
...
각 전이는 시각, 이전 상태, 다음 상태, 명령과 설명을 history에 추가한다. 기존 history는 수정하지 않는다.

상태 파일에는 전체 로그나 비밀값 대신 다음과 같은 최소 재현 증거만 저장한다.

- 선행 PR과 Merge commit
- Red, 집중 테스트, 전체 테스트의 명령·결론·시각
- PR 번호와 URL
- 리뷰 댓글 ID와 처리 결과
- `/merge-approved` 댓글 ID, 작성자, URL, 시각과 승인 대상 SHA
- Merge commit과 `main` 동기화 결과
