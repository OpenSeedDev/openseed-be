...
- 반복 횟수가 3회 이상이면 안전한 범위에서 테스트, 스크립트, `AGENTS.md`, Skill 또는 CI에 방지책을 자동 반영한다. 제품 정책이나 권한 확대가 필요할 때만 해당 PR을 차단하고 사용자 결정을 요청한다.
- PR 리뷰 수정은 가능하면 회귀 테스트를 추가하고 `review_comment_id`를 기록한다.
- 자동 복구는 같은 원인에 최대 3회까지만 수행하고, 초과 시 해당 작업만 `BLOCKED`로 둔다.
