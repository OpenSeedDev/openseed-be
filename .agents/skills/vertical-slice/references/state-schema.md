...
- Global Fast Build uses `delivery.strategy: global_stacked_pr` and `stack_root: VS-GLOBAL`.
  Its first task may have no parent PR because the configured integration branch is its base; every
  later task records the preceding global task and PR as its parent.
