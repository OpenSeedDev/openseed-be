...
class BacklogTest < Minitest::Test
  def setup
    @data = {
      ...
      "tasks" => [
        ...
        { "id" => "VS-004", "order" => 5, "title" => "four", "depends_on" => ["SETUP-01"], "resource_locks" => ["company"] }
      ]
    }
  end
  ...
  def test_selects_independent_ready_tasks_in_order
    data = load_data
    assert_equal %w[VS-001 VS-002 VS-004], Backlog.ready(data, merged: [], active: []).map { |task| task["id"] }
  end

  def test_respects_active_locks_and_worker_capacity
    data = load_data
    active = [{ "id" => "OTHER", "resource_locks" => ["member"] }]
    assert_equal %w[VS-002 VS-004], Backlog.ready(data, merged: [], active: active).map { |task| task["id"] }
  end
